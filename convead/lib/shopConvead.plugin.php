...
{
    // Добавляем настройку асинхронного режима
    private function getAsyncMode() {
        $plugin = wa('shop')->getPlugin('convead');
        $settings = $plugin->getSettings();
        return isset($settings['async_mode']) ? $settings['async_mode'] : true;
    }
    ...
    // Эмуляция cart_set_quantity и cart_add event
    public function routing($route = array())
    {
        $uri = waRequest::server('REQUEST_URI');
        if (strpos($uri, 'add') !== false and !empty($_POST['product_id'])) {
            $this->update_cart(array('cart_add' => true, 'product_id' => intval($_POST['product_id'])));
        }
        if (strpos($uri, 'save') !== false and !empty($_POST['quantity']) and !empty($_POST['id'])) {
            $this->update_cart(array('cart_set_quantity' => true, 'id' => intval($_POST['id']), 'quantity' => intval($_POST['quantity'])));
        }
    }
    ...
    public function update_cart($params = array())
    {
        ...
        {
            if (isset($products_cart_res[$params['id']])) {
                $products_cart_res[$params['id']]['quantity'] = $params['quantity'];
            }
        }
        ...
        foreach($products_cart_res as $product)
        {
            ...
            if ($product['sku_id'] != $product['product']['sku_id']) {
...
{$js_ready}
    ...
    // Добавляем метод для обработки очереди через cron
    public function processQueue()
    {
        if (!($tracker = $this->_include_tracker_anonym())) return false;
        return $tracker->processQueue();
    }
    ...
    private function _include_tracker_anonym()
    {
        ...
        $tracker->setAsyncMode($this->getAsyncMode());
    ...
    public static function _get_app_key()
    {
        ...
        foreach (siteHelper::getDomains(true) as $domain_id=>$domain) {
            if (waRequest::server('SERVER_NAME') == $domain['name']) break;
        }

        if (isset($settings['options']) and isset($settings['options']['domains']) and
            !empty($settings['options']['domains'][$domain_id]) and
            !empty($settings['options']['domains'][$domain_id]['api_key'])) {
            return $settings['options']['domains'][$domain_id]['api_key'];
        } else if (!empty($settings['options']['api_key'])) {
